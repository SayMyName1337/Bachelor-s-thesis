...
def main_window():
    ...
    layout = [
        ...
        [sg.Text('<Duration, Competition, Type, Crisis>, decision value: Prosit(Raise/Fall)')],
...
# Implementation of the program in response to user actions
    ...
    while True:
        ...
        if event == 'Run the algorithm':
            ...
            if text != []:
                window['-OUTPUT-'].update('')
                Test_dt = LIB(text,["Duration","Competition","Type", "Crisis"])
                output_res = ''
                print("Please standby\nThe RS1 algorithm is being launched ...")
                print('-' * 150)
                output_res += "U => " + str(Test_dt.getU()) + "\n"
                output_res += "X={x| Profit(x)=" + str(f"<{decision_value}>") + "}" + " => " + str(Test_dt.getX(decision_value)) + "\n"
